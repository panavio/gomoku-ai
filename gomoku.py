...
def is_bounded(board, y_end, x_end, length, d_y, d_x):
    ...
    x_start=x_end-(d_x*(length-1)) #=2-(1*3)
    y_start=y_end-(d_y*(length-1))
    if is_sequence_complete(board, colour, y_start, x_start, length, d_y, d_x):
        x_after=x_end+d_x
        x_before=x_start-d_x
        y_after=y_end+d_y
        y_before=y_start-d_y

        if is_sq_in_board(board, y_after, x_after) and is_sq_in_board(board, y_before, x_before):
        ...
        elif is_sq_in_board(board, y_after, x_after):
            if board[y_after][x_after]==" ":
                return "SEMI OPEN"
            return "CLOSED"

        elif is_sq_in_board(board, y_before, x_before):
            if board[y_before][x_before]==" ":
                return "SEMI OPEN"
            return "CLOSED"
...
#print(is_bounded([["","","",],["","w","",],["w","","",]],2,0,2,1,-1))
...
def detect_row(board, col, y_start, x_start, length, d_y, d_x):
    ...
    open_seq_count=0
    while is_sq_in_board(board,y,x)==True:
        if board[y][x]== col:
            if is_sequence_complete(board, col, y, x, length, d_y, d_x)==True:
                ...
                if is_sq_in_board(board,y,x):
                    if (is_bounded(board, y, x, length, d_y, d_x))=="OPEN":
                        open_seq_count+=1
                    elif (is_bounded(board, y, x, length, d_y, d_x))=="SEMI OPEN":
...
#print(detect_row([["w","","",],["","w","",],["","","w",]],"w",0,0,2,1,-1))
...
def detect_rows(board, col, length):
    ...
    for rows in range(len(board)):
        ...
        open_seq_count +=detect_row(board, col, rows, 0, length, 1, 1)[0]
        semi_open_seq_count+=detect_row(board, col, rows, 0, length, 1, 1)[1]

    for rows in range(1,len(board)):
        open_seq_count+=detect_row(board, col, rows, (len(board[0])-1), length, 1, -1)[0]
        semi_open_seq_count+=detect_row(board, col, rows, (len(board[0])-1), length, 1, -1)[1]
    ...
    for columns in range(len(board[0])):
        ...
        open_seq_count +=detect_row(board, col, 0, columns, length, 1, -1)[0]
        semi_open_seq_count+=detect_row(board, col, 0, columns, length, 1, -1)[1]
...
def test_detect_row():
    ...
    x = 5; y = 1; d_x = 0; d_y = 1; length = 3
    put_seq_on_board(board, y, x, d_y, d_x, length, "w")
    put_seq_on_board(board, 0, 5, d_y, d_x, 1, "b")
    print_board(board)
    print(detect_row(board, "w", 0,x,2,d_y,d_x))
...
def test_search_max2():
    board = make_empty_board(8)
    x = 4; y = 1; d_x = 0; d_y = 1; length = 4; col = 'w'
    put_seq_on_board(board, y, x, d_y, d_x, length, col)
    x = 0; y = 0; d_x = 1; d_y = 1; length = 3; col = 'b'
    put_seq_on_board(board, y, x, d_y, d_x, length, col)
    print_board(board)
